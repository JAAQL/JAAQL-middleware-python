...
QUERY__sign_up_insert = "INSERT INTO jaaql__sign_up (the_user, email_template, data_lookup_json) VALUES (:the_user, :template, :data_lookup_json) RETURNING key_a as invite_key, key_b as invite_poll_key"
QUERY__sign_up_poll = "SELECT key_a as invite_key, activated, the_user, closed, us.email FROM jaaql__sign_up su INNER JOIN jaaql__user us ON us.id = su.the_user WHERE (key_a = :invite_or_poll_key or key_b = :invite_or_poll_key) AND extract(epoch FROM (current_timestamp - su.created)) * 1000 < expiry_ms"
